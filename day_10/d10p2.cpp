...
int count_paths_dfs(vector<vector<int>>& map, int i, int j, set<pair<int,int>>& visited) {
    if (map[i][j] == 9) {
        return 1;
    }
    ...
    for (auto& dir : directions) {
        ...
        if (new_i >= 0 && new_i < map.size() && 
            ...
            paths += count_paths_dfs(map, new_i, new_j, visited);
            visited.erase({new_i, new_j});
        }
    }
...
int get_trail_rating(vector<vector<int>>& map, int start_i, int start_j) {
    set<pair<int,int>> visited;
    visited.insert({start_i, start_j});
    return count_paths_dfs(map, start_i, start_j, visited);
}
...
int main(){
    ...
    for (int i = 0; i < map.size(); i++){
        for (int j = 0; j < map[i].size(); j++){
            if (map[i][j] == 0){
                sum_scores += get_trail_rating(map, i, j);
            }
        }
    }
